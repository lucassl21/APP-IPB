...
from models import (
    ...
    AdminPasswordReset,
    TipoDoacao,
    TipoSolicitacao, # Adicionado
    DoadorInfo,
    BeneficiarioInfo,
    OracaoInfo, # Adicionado
    UserTypeChoice,
    Solicitacao,
    StatusSolicitacao,
    SolicitacaoUpdate,
...
# Atualização: Agora aceita "oracao" como tipo de usuário
...
def doador_info(cpf: str, doador_data: DoadorInfo, session: Session = Depends(get_session)):
    ...
    solicitacao = Solicitacao(
        tipo_solicitacao=TipoSolicitacao.doacao, # Alterado para usar o novo Enum
        descricao=f"Doação de {doador_data.tipo_doacao}: {doador_data.descricao}",
        cpf_usuario=cpf,
        usuario=user,
        status=StatusSolicitacao.pendente # Adicionado o status inicial
    )
    session.add(solicitacao)
    session.commit()
    session.refresh(solicitacao)
...
def beneficiario_info(cpf: str, beneficiario_data: BeneficiarioInfo, session: Session = Depends(get_session)):
    ...
    solicitacao = Solicitacao(
        tipo_solicitacao=TipoSolicitacao.beneficio, # Alterado para usar o novo Enum
...
# Novo: Endpoint para pedido de oração
